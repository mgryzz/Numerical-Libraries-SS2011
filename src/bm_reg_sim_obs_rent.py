...
iterations = 1000
file = open("bm_reg_sim_obs_rent.csv", "w")
...
for noObs in range(10,2500,10):
    ...
    gsl_process = subprocess.Popen(['test_gsl_reg_sim', '-nv', '-i', str(iterations), '-foms', '-f', 'data/miete03.asc', '-xs', '2', '-ys', str(noObs)],
                               stdout=subprocess.PIPE)
    gsl_output = gsl_process.communicate()

    nag_process = subprocess.Popen(['test_reg_sim', '-nv', '-i', str(iterations), '-foms', '-f', 'data/miete03.asc', '-xs', '2', '-ys', str(noObs)],
                               stdout=subprocess.PIPE)
